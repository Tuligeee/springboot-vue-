...
import com.mock.example.common.annotation.Excels;
import com.mock.example.common.config.ProjectConfig;
import com.mock.example.common.entity.Response;
import com.mock.example.common.exception.UtilException;
import com.mock.example.common.utils.file.FileTypeUtil;
import com.mock.example.common.utils.file.FileUtil;
import com.mock.example.common.utils.file.ImageUtil;
...
public class ExcelUtil<T> {
    ...
    /**
     * 直接从 File 对象导入，规避流读取在 Windows 下的锁定或不完整问题
     */
    public List<T> importExcel(File file) throws Exception {
        return importExcel(StringUtil.EMPTY, file);
    }
     ...
     * 流式导入（针对大数据量优化）
     */
    public List<T> importExcelStreaming(File file) {
        final List<T> list = new ArrayList<>();
        importExcelStreaming(file, batch -> list.addAll(batch), 1000); // 默认行批次
        return list;
    }
     ...
     */
    public void importExcelStreaming(File file, java.util.function.Consumer<List<T>> batchConsumer, int batchSize) {
        this.type = Excel.Type.IMPORT;
        createExcelField(); // 初始化 fields 集合

        // 构建列索引到 (Field, Excel) 的元数据映射
        final Map<Integer, Object[]> colMetadataMap = new HashMap<>(); // [Field, Excel]
        final Map<String, Integer> headerMap = new HashMap<>();
        final List<T> batchList = new ArrayList<>(batchSize);

        cn.hutool.poi.excel.ExcelUtil.readBySax(file, 0, new RowHandler() {
            @Override
            public void handle(int sheetIndex, int rowIndex, List<Object> rowList) {
                ...
                try {
                    ...
                    for (Map.Entry<Integer, Object[]> entry : colMetadataMap.entrySet()) {
                        int col = entry.getKey();
                        if (col >= rowList.size()) continue;
                        ...
                        Field field = (Field) metadata[0];
                        Excel attr = (Excel) metadata[1];
                        
                        Class<?> fieldType = field.getType();
                        Object result = val;

                        if (String.class == fieldType) {
                            result = Convert.toStr(val);
                        } else if (Integer.TYPE == fieldType || Integer.class == fieldType) {
                            result = Convert.toInt(val);
                        } else if (Long.TYPE == fieldType || Long.class == fieldType) {
                            result = Convert.toLong(val);
                        } else if (Double.TYPE == fieldType || Double.class == fieldType) {
                            result = Convert.toDouble(val);
                        } else if (BigDecimal.class == fieldType) {
                            result = Convert.toBigDecimal(val);
                        } else if (Date.class == fieldType) {
                            if (val instanceof String) {
                                result = DateUtil.parseDate(val);
                            } else if (val instanceof Double) {
                                result = org.apache.poi.ss.usermodel.DateUtil.getJavaDate((Double) val);
                            }
                        } else if (Boolean.TYPE == fieldType || Boolean.class == fieldType) {
                            result = Convert.toBool(val, false);
                        }

                        // 转换表达式处理 (readConverterExp / dictType)
                        if (StringUtils.isNotEmpty(attr.readConverterExp())) {
                            result = reverseByExp(Convert.toStr(result), attr.readConverterExp(), attr.separator());
                        } else if (StringUtils.isNotEmpty(attr.dictType())) {
                            result = reverseDictByExp(Convert.toStr(result), attr.dictType(), attr.separator());
                        }
                        
                        ReflectUtil.invokeSetter(entity, field.getName(), result);
                    }
                    if (hasData) {
    ...
    private List<T> parseSheet(String sheetName) throws Exception {
    ...
    public Response exportExcel() {
        ...
        try {
            ...
            wb.write(out);
            return new Response().okMsg(filename);
        } catch (Exception e) {
            log.error("导出Excel异常{}", e.getMessage());
    ...
    public void setCellVo(Object value, Excel attr, Cell cell) {
        if (Excel.ColumnType.STRING == attr.cellType()) {
            ...
            if (StringUtil.isNotNull(value)) {
                cell.setCellValue(StringUtil.contains(Convert.toStr(value), ".") ? Convert.toDouble(value) :
                        Convert.toInt(value));
            }
